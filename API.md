...
| `snippets` | list | `{"trigger", "label", "text", "bar", "color"}`; malformed entries are dropped here rather than becoming a render error later. `bar: true` also shows it as a one-tap button on the shortcuts bar, on every CLI, and then `trigger` may be empty — a bar-only snippet is never typed. `color` is a hex string tinting that pill, or `""` for the default accent tint; anything not a plain hex triple is dropped to `""` |
